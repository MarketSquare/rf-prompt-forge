...
def get_rf_package_names():
    ...
    try:
        ...
        package_names = re.findall(r'>robotframework-([^<]+)<', response.text, re.IGNORECASE)
        full_names = sorted(list(set([f"robotframework-{name.lower()}" for name in package_names])))
...
def main():
    ...
    if packages_to_fetch:
        failed_packages = []
        with ThreadPoolExecutor(max_workers=5) as executor: # Reduced workers to be gentler on the API
            print(f"Fetching stats for {len(packages_to_fetch)} packages...")
            future_to_package = {executor.submit(get_package_downloads, name): name for name in packages_to_fetch}
