...
def load_and_clean_libdoc(lib_name: str) -> dict:
    ...
    if not lib_data_file.exists():
        print(
            f"ERROR: Data file not found for {lib_name}. Run generate_libdoc.py first."
        )
        return None
    ...
    for kw in lib_data.get("keywords", []):
        kw["doc"] = re.sub(
            r"\n\n\[https://forum\.robotframework\.org/t//\d+\|Comment >>\]",
            "",
            kw["doc"],
        ).strip()
...
def main():
    ...
    for lib_config in libraries:
        ...
        if not all([template_name, output_file, category]):
            print(
                f"Warning: Skipping {lib_config['name']} due to missing config (template, output_file, or category)."
            )
            continue
